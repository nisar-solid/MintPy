...
def diff_timeseries(file1, file2, out_file, force_diff=False, max_num_pixel=2e8):
    ...
    if ref_y and ref_x:
        ...
        ref_val = (
            readfile.read(file2, datasetName=date_list_shared, box=ref_box)[0]
            * unit_fac
        )
    else:
        ref_val = None
    ...
    for i, box in enumerate(box_list):
        ...
        data2 = (
            readfile.read(file2, datasetName=date_list_shared, box=box)[0] * unit_fac
        )

        if ref_val is not None:
            print(
                f"* referencing data from {os.path.basename(file2)} to y/x: {ref_y}/{ref_x}"
            )
            data2 -= np.tile(
                ref_val.reshape(-1, 1, 1), (1, data2.shape[1], data2.shape[2])
            )

        if ref_date:
            print(
                f"* referencing data from {os.path.basename(file2)} to date: {ref_date}"
            )
        ...
        data[mask] = 0.0  # Do not change zero phase value
        del data2
...
def diff_timeseries_and_velocity(file1, file2, out_file, max_num_pixel=2e8):
    ...
    if ds_names != ["velocity"]:
        print(
            "WARNING: ONLY velocity is supported, ignore the following datasets and continue:"
        )
    ...
    for i, box in enumerate(box_list):
        ...
        if ref_val is not None:
            print(
                f"* referencing velocity to y/x: {ref_y}/{ref_x} with value of {np.squeeze(ref_val)*100:.2f} cm/year"
            )
            velo -= ref_val
        ...
        print(
            f"* reconstructing time-series from {os.path.basename(file2)} with model {model}"
        )
        ...
        if "REF_DATE" in atr1.keys():
            print(
                f'* referencing time-series from file2: {os.path.basename(file2)} to date: {atr1["REF_DATE"]}'
            )
        ...
        writefile.write_hdf5_block(
            out_file, data=data, datasetName="timeseries", block=block
        )
...
def diff_ifgram_stack(file1, file2, out_file):
    ...
    if "unwrapphase" in ds_name.lower():
        ref_yx1 = (obj1.refY, obj1.refX)
        ref_yx2 = (obj2.refY, obj2.refX)
        has_ref1 = all(v is not None for v in ref_yx1)
        has_ref2 = all(v is not None for v in ref_yx2)

        ref_yx = None
        if has_ref1 and has_ref2:
            ref_yx = ref_yx1
            if ref_yx1 != ref_yx2:
                print(
                    f"file1/file2 reference pixels differ: {ref_yx1} vs {ref_yx2}; use file1 reference pixel."
                )
        elif has_ref1:
            ref_yx = ref_yx1
            print(f"only file1 has a reference pixel; use {ref_yx1} for both stacks.")
        elif has_ref2:
            ref_yx = ref_yx2
            print(f"only file2 has a reference pixel; use {ref_yx2} for both stacks.")

        if ref_yx is not None:
            print(f"referencing both stacks to pixel {ref_yx} ...")
            ref1 = data1[:, ref_yx[0], ref_yx[1]]
            ref2 = data2[:, ref_yx[0], ref_yx[1]]
            for i in range(data1.shape[0]):
                data1[i, :][data1[i, :] != 0.0] -= ref1[i]
                data2[i, :][data2[i, :] != 0.0] -= ref2[i]
...
def diff_ifgram_and_timeseries(unw_file, ts_file, cor_file):
    ...
    if dunit.startswith("rad"):
        ...
        delay *= -4.0 * np.pi / float(atr["WAVELENGTH"])
    ...
    for ext in ["xml", "vrt"]:
        ...
        if os.path.isfile(unw_meta_file):
            ...
            with open(meta_file) as f:
                s = f.read()
            s = s.replace(os.path.basename(unw_file), os.path.basename(cor_file))
...
def diff_file(file1, file2, out_file, force_diff=False, max_num_pixel=2e8):
    ...
    if k1 == "timeseries":
        if k2 not in ["timeseries", "giantTimeseries", "velocity"]:
            print(
                "If the first file is timeseries, the following file must be either timeseries or velocity."
            )
        ...
        if len(ds_names) < 1:
            raise ValueError(
                f"No common datasets found among files:\n{[file1] + file2}"
            )
        ...
        for ds_name in ds_names:
            ...
            for i, fname in enumerate(file2):
                ...
                if ds_name == "velocity":
                    ...
                    if ref_y and ref_x:
                        print(
                            f"* referencing data from {os.path.basename(file2[0])} to y/x: {ref_y}/{ref_x}"
                        )
